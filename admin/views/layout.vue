...
<div class="top">
  ...
  <ul class="top-right">
    ...
    <div class="right-menu">
      ...
      <ul>
        <router-link to="/admin"><li><img src='~src/assets/img/home_pc.png'><span> 回到首页</span></li></router-link>
        <li v-on:click="changePass"><img src='~src/assets/img/pass.png'><span>修改密码</span></li>
        <li v-on:click="casheUpdate"><img src='~assets/img/fresh.png'><span>刷新缓存</span></li>
        <li v-on:click="logout"><img src='~src/assets/img/logout.png'><span>安全退出</span></li>
      </ul>
    </div>
  </ul>
...
<div class="view">
  <div class="left">
      <div class="info">
        <div class="img">
          <img class="headpic" v-if="head" :src="head">
          <img class="headpic" v-else src="~src/assets/img/head_portrait_default.png">
        </div>
        <div class="text">
          <div>欢迎你:{{name}}</div>
          <div>组:{{groupname}}</div>
...
export default{
  data() {
    return {
      name: models.user.name, // 账号名
      head: models.user.head, // 账号名
      groupname: models.user.groupname, // 用户组
  ...
  methods: {
    ...
    updateMainMenu() {
      ...
      adminRoute.forEach(route_item => {
        ...
        if (route_item.meta.auth) {
          if (models.user.authDic[route_item.meta.auth]) {
    ...
    updateSubMenu() {
      ...
      selectitem.children.forEach(subitem => {
        ...
        if (subitem.meta.auth) {
          const authinfo = models.user.authDic[subitem.meta.auth]
          if (authinfo) {
            auth_right = authinfo.auth;
          }
        }
        if (auth_right) {
    ...
    logout() {
      post('login', 'logout').then(() => {
        ...
        window.localStorage.setItem('utokenid', '')
        ...
        callback: (action, inst) => {
          ...
          if (action === 'confirm') {
            ...
            if ((inst.inputValue.trim() || '') === '') {
              ...
              post('user', 'changePassword', { password: inst.inputValue.trim() })
                .then(() => {
